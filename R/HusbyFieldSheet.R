...
#' @param verbose logical, whether to write messages while the function runs
...
HusbyFieldSheet <- function(path, SheetNames, parallel = FALSE, cores = NULL, verbose = TRUE){
  ...
  if(!parallel){
    for (i in 1:lengthLoop) {
      ...
      for (k in 2:nrow(PinpointData)) {
        for (l in 1:nrow(husbyDT)) {
          if (PinpointData[k, 1] == husbyDT[l, 5]) {
            ...
            husbyDT[l, 9] = sum(str_count(PinpointCount,
                                          "x"))
            duplicates <- append(duplicates, husbyDT[l,
                                                     5])
          }
        }
      }
      ...
      husbyDT <- cbind(husbyDT, extraData[!names(extraData) %in%
                                            names(husbyDT)])
      countSpecies <- matrix(count(unique(husbyDT[5])), ncol = 1,
                             nrow = nrow(husbyDT))
      CountSpeciesData <- data.frame(countSpecies)
      husbyDT <- cbind(husbyDT, CountSpeciesData[!names(CountSpeciesData) %in%
                                                   names(husbyDT)])
      husbyDT$Species <- stringr::str_replace_all(husbyDT$Species, "\\u00e5", "aa")
      husbyDT$Species <- stringr::str_replace_all(husbyDT$Species, "\\u00f8", "oe")
      husbyDT$Species <- stringr::str_replace_all(husbyDT$Species, "\\u00e6", "ae")
      husbyDT$Species <- stringr::str_replace_all(husbyDT$Species, "\\u00eb",
                                         "e")
      husbyDT$Type <- stringr::str_replace_all(husbyDT$Type, "\\u00e5", "aa")
      husbyDT$Type <- stringr::str_replace_all(husbyDT$Type, "\\u00f8", "oe")
      husbyDT$Type <- stringr::str_replace_all(husbyDT$Type, "\\u00e6", "ae")
      husbyDT$Type <- stringr::str_replace_all(husbyDT$Type, "\\u00eb", "e")
      if(verbose){
        print(SheetNames[i])
      }
      listOfDFs <- list()
    ...
    new <- foreach(i=1:lengthLoop, .errorhandling = "pass", .packages = c("readxl", "dplyr", "stringr"), .combine = rbind) %dopar%{
      ...
      husbyDT$Species <- stringr::str_replace_all(husbyDT$Species, "\\u00f8", "oe")
      husbyDT$Species <- stringr::str_replace_all(husbyDT$Species, "\\u00e6", "ae")
      husbyDT$Species <- stringr::str_replace_all(husbyDT$Species, "\\u00eb", "e")
